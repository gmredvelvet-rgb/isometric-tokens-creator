/**
 * Isometric Token Creator — cliente de licencia.
 *
...
export const L = (key, data) => {
  // Las claves de este módulo son planas ("ITC.License.Connect"), no un
...
export default class LicenseClient {
  ...
  async initialize() {
    ...
    if ( this.#accessToken && this.#isAccessTokenValid() ) {
      try {
        ...
        Logger.warn("Token de licencia almacenado rechazado", err);
    ...
    if ( this.#refreshToken ) {
      try {
        ...
        if ( this.#isTransient(err) ) {
          Logger.warn("Servidor de licencias inaccesible — se conservan las credenciales", err);
          this.#startHeartbeat();
          return this.isTrusted;
        }
        Logger.warn("Refresco de licencia rechazado de forma definitiva", err);
        this.#clearStoredTokens();
      }
    }
    return this.isTrusted;
  ...
  #onHeartbeatFailure(err) {
    ...
    if ( !this.#degraded ) {
      this.#degraded = true;
      Logger.warn("Falló el heartbeat de licencia", err);
    }
  ...
  async #setWorldLicensed(licensed) {
    ...
    try {
      ...
      Logger.warn("No se pudo persistir el flag de licencia del mundo", err);
    }
  }
