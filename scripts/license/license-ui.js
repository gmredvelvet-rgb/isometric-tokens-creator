/**
 * Isometric Token Creator — avisos de licencia y recordatorio de prueba.
 *
...
export default class LicenseUI {
  ...
  static show({ autoHide = false } = {}) {
    ...
    card.id = CARD_ID;
    card.className = `itc-license-card ${licensed ? "is-active" : "is-trial"}`;
  ...
  static #header(badge = "") {
    return `
      <header>
        ...
        ${badge ? `<span class="itc-license-badge">${badge}</span>` : ""}
      </header>`;
  }
  ...
  static #trialMarkupGM() {
    return `
      ...
      <p class="itc-license-status">${L("TrialIntro")}</p>
      <p class="itc-license-note">${L("TrialPitch")}</p>
      ${LicenseUI.migrationNotice()}
      <button type="button" data-action="connect" class="itc-license-primary">
        <i class="fa-brands fa-patreon" aria-hidden="true"></i> ${L("Connect")}
      </button>
      <button type="button" data-action="code">${L("HaveCode")}</button>
      <button type="button" data-action="dismiss" class="itc-license-dismiss">${L("Later")}</button>`;
  }
  ...
  static #trialMarkupPlayer() {
    return `
      ${LicenseUI.#header(L("TrialBadge"))}
      <p class="itc-license-status">${L("TrialIntroPlayer")}</p>
      <p class="itc-license-note">${L("TrialPlayerNote")}</p>
      <button type="button" data-action="dismiss" class="itc-license-dismiss">${L("Close")}</button>`;
  }
  ...
  static #activeMarkup() {
    ...
    return `
      ${LicenseUI.#header()}
      <p class="itc-license-status">${status}</p>
      <p class="itc-license-note">${L("ReleaseHint")}</p>
...
export function registerLicenseMenu() {
  game.settings.registerMenu(MODULE_ID, SETTINGS.LICENSE_MENU, {
    name: "ITC.Settings.License.Name",
    label: "ITC.Settings.License.Label",
    hint: "ITC.Settings.License.Hint",
